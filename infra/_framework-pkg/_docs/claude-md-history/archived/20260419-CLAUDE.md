...
**At the start of every session, read `docs/ai-screw-ups/README.md`.** It documents recurring mistakes made by Claude in this repo. Do not repeat them.
...
- `query-unifi-switch` — live switch port data (link speed + MAC table). In `scripts/ai-only-scripts/query-unifi-switch/`. Use `SWITCH_FILTER=Flex ./run` to filter by switch name. **Use this instead of one-off curl commands whenever you need to know which NIC/MAC is on which switch port.**
...
**Every MaaS automation fix requires a new numbered plan: `docs/ai-plans/maas-snafu-N.md`.**

- Before writing any MaaS fix: check the highest existing snafu number in `docs/ai-plans/` and `docs/ai-plans/archived/`. Increment by 1.
...
- **Planning (ai-plans)**: for non-trivial multi-file changes, use `/doit` to write a plan to `docs/ai-plans/<task-name>.md` BEFORE coding. This allows the user to clear context between planning and implementation without losing the plan. Plan file format: files to create/modify, design decisions, exact code strategy. Commit the plan, surface open questions, then clear context and execute.
